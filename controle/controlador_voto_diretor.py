...
class ControladorVotoDiretor:
    ...
    def adicionar_voto(self):
        try:
            ...
            diretor_obj = self.__controlador_sistema.controlador_diretor.pegar_diretor_por_nome(dados_voto["indicado"])
            if not diretor_obj:
                self.__tela_voto.mostrar_mensagem("Diretor não indicado.")
                return
            ...
            dados_voto["indicado"] = diretor_obj
            ...
            self.__voto_diretor_DAO.add(voto)
    ...
    def alterar_voto(self):
        ...
        diretor_obj = self.__controlador_sistema.controlador_diretor.pegar_diretor_por_nome(novos_dados_voto["indicado"])
        if not diretor_obj:
            self.__tela_voto.mostrar_mensagem("Diretor não indicado.")
            return
    ...
    def remover_voto(self):
        ...
        if not voto:
            return
        self.__voto_diretor_DAO.remove(voto.membro, voto.indicado)
    ...
    def verificar_diretor_indicado(self, dados_voto):
        ...
        nomes_diretores = [diretor.nome.strip().title() for diretor in diretores_indicados]
        if nome_indicado not in nomes_diretores:
    ...
    def verificar_voto_existente(self, dados_voto):
        ...
        for voto in self.__voto_diretor_DAO.get_all():
    ...
    def gerar_relatorio_por_ano(self):
        ano = self.__tela_voto.buscar_voto_por_ano()
        votos_filtrados = [voto for voto in self.__voto_diretor_DAO.get_all() if voto.ano == ano]
    ...
    def gerar_relatorio_por_categoria(self):
        categoria = self.__tela_voto.buscar_voto_por_categoria().strip().title()
        votos_filtrados = [
            voto for voto in self.__voto_diretor_DAO.get_all()
    ...
    def pegar_votos_em_diretores(self):
        return self.__voto_diretor_DAO.get_all()
    ...
    def abrir_tela_voto_diretor(self):
        ...
        while True:
            try:
                op = self.__tela_voto_diretor.tela_votos_em_diretores()
                if op in opcoes:
                    opcoes[op]()
                else:
                    self.__tela_voto_diretor.mostrar_mensagem("Opção inválida!")
            except Exception as e:
                self.__tela_voto_diretor.mostrar_mensagem(f"Erro inesperado: {e}")
    ...
    def abrir_filtro_diretores(self):
        ...
        while True:
            try:
                op = self.__tela_voto_diretor.tela_filtros_de_relatorios()
                if op in opcoes:
                    opcoes[op]()
                else:
                    self.__tela_voto_diretor.mostrar_mensagem("Opção inválida!")
